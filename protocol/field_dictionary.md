...
| sensor_layout_version | string | 是 | layout_6p4t_v1 | 传感器布局版本 |
...
| temperature | number 数组 | 是 | 长度 4，每项 -40.00～125.00℃ | 四个温度通道；风险依据同区温差和个人基线 |
...
物理位置以 layout_6p4t_v1 为准，P1～P6 的上述语义是当前唯一有效定义。左右脚按解剖方向镜像，同一通道始终代表同一解剖区域；P2 始终表示该脚前掌外侧，P4 始终表示前掌内侧。代码使用数组索引和 sensor_layout_version，不得在业务逻辑中到处写死物理位置。
...
|---:|---|---|
| 0 | T1 | 前掌外侧 |
| 1 | T2 | 拇趾/第一跖骨头邻近的前掌内侧 |
| 2 | T3 | 足跟中央 |
| 3 | T4 | 中足中央 |

T1～T4 与压力通道采用相同的镜像规则：左右脚按解剖方向镜像，同一通道始终代表同一解剖区域。
...
3. 压力风险先转换为载荷占比、左右同区差异和相对个人基线的变化量，再使用配置中的工程阈值；不得对原始压力直接使用统一人体阈值。
4. 温度风险比较左右脚镜像同区域温差和相对个人基线的升温，不只判断某个绝对温度。
5. 编码使用四舍五入到最近整数；恰好位于中点时远离 0 取整，固件和测试工具必须使用一致规则。
