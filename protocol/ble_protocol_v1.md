...
|---|---|---|---|---|
| SensorData | 7d2f0001-5a6b-4c7d-8e9f-102030405060 | Notify | ESP32 → App | 每次 Notify 恰好一条 60 字节帧 |
...
1. App 连接后请求 MTU 247。
2. SensorData 需要协商 MTU 至少为 63；否则不得开始流式传输。
...
- 每次 SensorData Notify 必须且只能发送一条完整的 60 字节帧。
- CRC：CRC-16/CCITT-FALSE，对偏移 0～57 计算，CRC 字段本身不参与计算。
...
| 3 | 1 | uint8 | layout_id | 2=layout_6p4t_v1 |
...
| 37 | 8 | 4×int16 | temperature | round_half_away_from_zero(℃×100) |
| 45 | 6 | 3×int16 | acceleration | round_half_away_from_zero(m/s²÷9.80665×1000)，单位 mg |
| 51 | 6 | 3×int16 | gyroscope | round_half_away_from_zero(°/s×10) |
| 57 | 1 | uint8 | battery | 0～100 |
| 58 | 2 | uint16 | crc16 | CRC 值按小端序写入，低字节在前 |
...
CRC 对 SensorData 偏移 0～57 的 58 个字节按顺序计算。计算结果作为 uint16 小端序写入偏移 58～59，即先写低字节，再写高字节。
