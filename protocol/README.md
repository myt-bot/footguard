...
本次 1.1 修订发生在真实 BLE 联调前，保留 protocol_version=1、layout_6p3t_v1 和 58 字节 SensorData 布局，补充 MTU、CRC 测试向量、DeviceStatus、DeviceCommand、TimeSync、ACK、幂等和错误处理规则。此前尚未实现的 v1 草案由本修订版取代。
...
- REVISION_NOTES_v1_1.md：本次真实 BLE 联调修订摘要与兼容性说明。
...
1. ESP32 按 BLE 58 字节二进制帧发送 SensorData。
...
1. App 请求 MTU 247。
2. SensorData 每次 Notify 恰好发送一条 58 字节帧。
